		...
		while(!queue.isEmpty()){
			...
			for (int i = -1; i <= 1; ++i){
				if (start.lastStep + i != 0){
					...
					int destinationStone = start.stone + start.lastStep + i;
					if (destinationStone == stones[stones.length - 1]) {
						return true;
					}
					
					if (stonesSet.contains(destinationStone)){
						queue.push(new Jump(destinationStone, start.lastStep + i));
					}
				}
			}
		}
