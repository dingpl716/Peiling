...
public class WordLadderII {
	...
	private static class Word2Word{
		...
		//按照3的想法，需要比较word2word，但是按照5的想法已经不需要比较了，
		//所以也不需要hashcode和equals方法了
		@Override
		public int hashCode() {
			final int prime = 31;
			int tmp = 0;
     ...
     * 能跳到终点,则降至加入到endpoints里面，并返回true; 如果不能
     * 而又是在dict里面且没访问过的元素，则加入到nq里面，并且在met里面
     * 记录下来
     * @param cq Current queue, get every element from this queue
     * @param nq Next queue, add elements to this queue
     * @param end
     * @param met 
     * @param dict
     * @param endPoints
     * @return
     */
    private boolean jumpOneStep(Queue<Word2Word> cq, Queue<Word2Word> nq, String end,
    		Map<String, Integer> met, HashSet<String> dict, ArrayList<Word2Word> endPoints) {
    	boolean metEnd = false;
    	
    	while(!cq.isEmpty()) {
			Word2Word prev = cq.poll();
			//当前到达的单词
