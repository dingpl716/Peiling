...
public class Main {
	...
	static KeyPair pk_scrooge = null;
	public static KeyPair getScroogePK() throws NoSuchAlgorithmException {
		if (pk_scrooge == null)
    ...
    private static void test1() throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        /*
    ...
    private static void test2() throws NoSuchAlgorithmException {
    	/*
         * Generate key pairs, for Scrooge, Alice & Bob
         */
        KeyPair pk_scrooge = KeyPairGenerator.getInstance("RSA").generateKeyPair();
         ...
         * Create Genesis block: No TXs, but 1 Coinbase
         */
    	Block genesis = new Block(null, pk_scrooge.getPublic(), "genesis");
    	genesis.finalize();
    	BlockChain bc = new BlockChain(genesis);
    	BlockHandler bh = new BlockHandler(bc);
    	
    	Block b1 = addToBlock(bh, genesis, "1");
    	System.out.println(bc.getMaxHeightBlock().getHash().equals(b1.getHash()));
    	
    	Block b2 = addToBlock(bh, genesis, "2");
    	System.out.println(bc.getMaxHeightBlock().getHash().equals(b1.getHash()));
    	
    	Block b3 = addToBlock(bh, b2, "3");
    	System.out.println(bc.getMaxHeightBlock().getHash().equals(b3.getHash()));
    	
    	Block b4 = addToBlock(bh, b1, "4");
    	System.out.println(bc.getMaxHeightBlock().getHash().equals(b3.getHash()));
    }
    
    private static Block addToBlock(BlockHandler bh, Block prevBlock, String newBlockId) throws NoSuchAlgorithmException {
    	/*
    	 * Create new block3 chained to block1 with tx alice -> bob
             */
    	Block newBlock = new Block(prevBlock.getHash(), getScroogePK().getPublic(), newBlockId);
    	newBlock.finalize();

    	System.out.println("Block " + newBlockId +" Added ok: " + bh.processBlock(newBlock));
    	return newBlock;
    }
}